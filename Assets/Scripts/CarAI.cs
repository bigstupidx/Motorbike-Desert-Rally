...
public class CarAI : MonoBehaviour {
	...
	void FixedUpdate () 
	{
		...
		if (Mathf.Abs(gameObject.GetComponent<Rigidbody>().velocity.x) > maxVelocity)
		{
			if(vel.x<0f)
				vel.x = -maxVelocity;
			else
		...
		if (Mathf.Abs (gameObject.GetComponent<Rigidbody>().velocity.z) > maxVelocity)
		{
			if(vel.z<0f)
				vel.z = -maxVelocity;
			else
		...
		gameObject.GetComponent<Rigidbody>().AddForce((targetDir.normalized+additionalForce) * movementSpeed);
	}
